...
int main(int argc, char *argv[])
{
    ...
    if (g_node_id)
    {
        g_thread_cnt -= g_batching_thread_cnt;
        g_total_thread_cnt -= g_batching_thread_cnt;
        g_this_total_thread_cnt -= g_batching_thread_cnt;
        g_this_thread_cnt -= g_batching_thread_cnt;
        g_batching_thread_cnt = 0;
    }
...
#if SGX
    enclave_seq_number_sock = connect_to_enclave("ipc:///tmp/sgx_seq.ipc");
    enclave_prepare_sock = connect_to_enclave("ipc:///tmp/sgx_prepare.ipc");
    enclave_commit_sock = connect_to_enclave("ipc:///tmp/sgx_commit.ipc");
    enclave_exec_sock = connect_to_enclave("ipc:///tmp/sgx_exec.ipc");
#endif
  ...
  */
    ...
    fflush(stdout);
#if SGX
    send_message_to_enclave(TEE_TERMINATE);
#endif
    // Free things
    // tport_man.shutdown();
