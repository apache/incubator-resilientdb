...
* A complete refactoring of the code base such that (1) the core engine code is now moved to the platform folder, including the formwork architectures and protocols implementations;
  (2) the API/interface-related code are moved to the service folder, including UTXO, smart contact and key-value interface; 
  (3) the python SDK code is move to different repository [here](https://github.com/resilientdb/sdk).
