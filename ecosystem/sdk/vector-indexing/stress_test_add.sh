...
echo "=== Stress Test Started: Adding $NUM_ITERATIONS items sequentially ===" | tee -a "$OUTPUT_FILE"
...
do
    ...
    rand_idx2=$((RANDOM % ${#words[@]}))
    text="Test entry #$i: ${words[$rand_idx]} ${words[$rand_idx2]} related data with random seed $RANDOM"
    ...
    echo "Adding data: '$text'" >> "$OUTPUT_FILE"
    ...
    python3 kv_vector.py --add "$text" >> "$OUTPUT_FILE" 2>&1
    ...
    if [ $exit_code -ne 0 ]; then
        ...
        msg="❌ [CRITICAL FAIL] vector_add.py crashed at step $i."
        echo "$msg" | tee -a "$OUTPUT_FILE"
        exit 1
    else
        # Log the individual duration ONLY to the file
        echo "⏱️  Add operation took: ${duration} ms" >> "$OUTPUT_FILE"
    fi
...
echo "   Total time spent on 'add' operations: ${total_duration} ms" | tee -a "$OUTPUT_FILE"
