  ...
  const badgeContent = (
    <Badge
      ...
      className={`text-xs ${size === "sm" ? "px-1.5 py-0.5" : "px-2 py-1"} ${
        clickable ? "cursor-pointer hover:bg-primary/20 transition-colors" : ""
      } ${className}`}
    ...
    >
      {showIcon && (
        <FileText className={`${size === "sm" ? "h-2.5 w-2.5" : "h-3 w-3"} mr-1`} />
      )}
      <span className="truncate">{shortName}</span>
      {clickable && (
        <ExternalLink className={`${size === "sm" ? "h-2.5 w-2.5" : "h-3 w-3"} ml-1 opacity-60`} />
      )}
    </Badge>
  );
  ...
  return (
    <div className={`flex flex-wrap gap-1 ${className}`}>
      ...
      {hiddenCount > 0 && (
        <Tooltip>
          <TooltipTrigger asChild>
            <Badge variant="outline" className={`text-xs ${size === "sm" ? "px-1.5 py-0.5" : "px-2 py-1"}`}>
              +{hiddenCount}
            </Badge>
          </TooltipTrigger>
          <TooltipContent>
...
export type { DocumentSource, DocumentSourceBadgeProps, MultiDocumentSourceBadgeProps, SourceAttributionProps };
