...
const MultiDocumentSelector = memo<MultiDocumentSelectorProps>(({
  ...
  const isDocumentSelected = useCallback((doc: Document) => {
    return selectedDocuments.some(selected => selected.id === doc.id);
  }, [selectedDocuments]);

  // Handle document toggle
  const handleDocumentToggle = useCallback((doc: Document) => {
  ...
  }, [isDocumentSelected, onDocumentSelect, onDocumentDeselect, maxSelections, selectedDocuments.length]);
  ...
  }, [selectedDocuments.length, documents.length, onSelectAll, onDeselectAll]);
      ...
      <div className="space-y-2">
        ...
        {showSelectAll && (
          <Button
            ...
            className="w-full"
            aria-label={selectionStats.allSelected ? "Deselect all documents" : "Select all documents"}
          >
