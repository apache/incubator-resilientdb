    ...
    try {
      ...
      while (true) {
        ...
        const currentTextContent =
          separatorIndex !== -1
            ? rawResponseData.substring(0, separatorIndex)
            : rawResponseData;
      ...
      if (parts.length > 1 && parts[1]) {
        try {
          ...
          console.error(
            "Failed to parse source nodes JSON:",
            e,
            "JSON string:",
            parts[1],
          );
    ...
    <div className="flex flex-col text-white font-sans min-h-screen">
      <main className=" overflow-y-auto py-6 flex justify-center items-center mt-[60px] flex-grow">
        <div className="w-full max-w-5xl px-4 space-y-4">
          <div
            className={`rounded-xl border border-white/10 h-[65vh] overflow-y-auto ${messages.length > 0 ? "bg-black/20 backdrop-blur-sm" : "hidden"} p-6 space-y-4`}
          >
            {messages.map((msg) => (
              <div
                key={msg.id}
