...
#include "executor/global/global_executor.h"
...
GlobalExecutor::GlobalExecutor(std::unique_ptr<Storage> storage)
    : storage_(std::move(storage)) {}

std::unique_ptr<google::protobuf::Message> GlobalExecutor::ParseData(
...
std::unique_ptr<std::string> GlobalExecutor::ExecuteRequest(
...
std::unique_ptr<std::string> GlobalExecutor::ExecuteData(
    const std::string& request) {
...
void GlobalExecutor::Set(const std::string& key, const std::string& value) {
  storage_->SetValue(key, value);
}

std::string GlobalExecutor::Get(const std::string& key) {
  return storage_->GetValue(key);
}

std::string GlobalExecutor::GetAllValues() { return storage_->GetAllValues(); }

// Get values on a range of keys
std::string GlobalExecutor::GetRange(const std::string& min_key,
                                 const std::string& max_key) {
  return storage_->GetRange(min_key, max_key);
}

void GlobalExecutor::SetWithVersion(const std::string& key,
                                const std::string& value, int version) {
  storage_->SetValueWithVersion(key, value, version);
}

void GlobalExecutor::GetWithVersion(const std::string& key, int version,
                                ValueInfo* info) {
...
void GlobalExecutor::GetAllItems(Items* items) {
...
void GlobalExecutor::GetKeyRange(const std::string& min_key,
...
void GlobalExecutor::GetHistory(const std::string& key, int min_version,
...
void GlobalExecutor::GetTopHistory(const std::string& key, int top_number,
                               Items* items) {
  const std::vector<std::pair<std::string, int>>& ret =
