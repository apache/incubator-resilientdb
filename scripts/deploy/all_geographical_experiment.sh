...
for num_london in "${num_londons[@]}"; do
    ...
    for protocol in "${protocols[@]}"; do
        tput_str+=$(head -n 1 ./plot_data/geographical_throughput/${protocol}_${num_london}.data | awk '{print $1}')" "
        lat_str+=$(head -n 1 ./plot_data/geographical_latency/${protocol}_${num_london}.data | awk '{print $1}')" "
    done
