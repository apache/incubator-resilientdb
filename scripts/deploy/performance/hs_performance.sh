...
/root/nexres/bazel-bin/benchmark/protocols/pbft/kv_service_tools $config_file
done
...
for ip in ${iplist[@]};
do
  echo "scp -i ${key} root@${ip}:/root/${server_bin}.log ./${ip}_log"
  `scp -i ${key} root@${ip}:/root/${server_bin}.log result_${ip}_log` 
done
