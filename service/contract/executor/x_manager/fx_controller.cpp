#include "service/contract/executor/x_manager/fx_controller.h"
...
FXController::FXController(DataStorage* storage, int window_size)
...
// ========================================================
const ModifyMap& FXController::GetChangeList(int64_t commit_id) {
  // read lock
  return std::move(changes_list_[commit_id]);
}

void FXController::Clear(int64_t commit_id) {
#ifdef CDebug
...
void FXController::StoreInternal(const int64_t commit_id,
                                 const uint256_t& address,
                                 const uint256_t& value, int version) {
  // LOG(ERROR)<<"store commit:"<<commit_id<<" address:"<<address;
...
uint256_t FXController::LoadInternal(const int64_t commit_id,
                                     const uint256_t& address, int version) {
  // LOG(ERROR)<<"load commit:"<<commit_id<<" address:"<<address;
  if (aborted_[commit_id]) {
    throw eevm::Exception(eevm::Exception::Type::outOfGas, "Get lock fail");
    return 0;
  }
...
void FXController::Store(const int64_t commit_id, const uint256_t& address,
                         const uint256_t& value, int version) {
  StoreInternal(commit_id, address, value, version);
}

uint256_t FXController::Load(const int64_t commit_id, const uint256_t& address,
                             int version) {
  return LoadInternal(commit_id, address, version);
}

bool FXController::Remove(const int64_t commit_id, const uint256_t& key,
                          int version) {
...
void FXController::AddRedo(int64_t commit_id) {
...
void FXController::AddDataToChangeList(std::vector<Data>& change_set,
                                       const Data& data) {
  if (change_set.empty()) {
...
int FXController::GetLastR(const uint256_t& address, int addr_idx, int idx) {
#ifdef CDebug
  LOG(ERROR)
      << "get last r, lp size:"
      << (lastr_.find(addr_idx) == lastr_.end() ? 0 : lastr_[addr_idx].size())
      << " address:" << address << " addr idx:" << addr_idx << " lw size:"
      << (lastw_.find(addr_idx) == lastw_.end() ? 0 : lastw_[addr_idx].size());
#endif
  auto it = lastr_.find(addr_idx);
  if (it == lastr_.end() || it->second.empty()) {
    // LOG(ERROR)<<"no last:"<<address;
    if (lastw_.find(addr_idx) == lastw_.end() || lastw_[addr_idx].empty()) {
      return -1;
    }
    auto pit = lastw_[addr_idx].begin();
    if (*pit == idx) pit++;
    if (pit == lastw_[addr_idx].end()) {
      return -1;
    }
    return *pit;

    if (*lastw_[addr_idx].begin() == 0) {
      LOG(ERROR) << "get last r size:" << lastw_[addr_idx].size();
    }
    if (*lastw_[addr_idx].begin() == 0 || *lastw_[addr_idx].begin() == idx) {
      LOG(ERROR) << "get last w:" << *lastw_[addr_idx].begin()
                 << " size:" << lastw_[addr_idx].size();
    }
...
int FXController::GetLastW(const uint256_t& address, int addr_idx, int idx) {
#ifdef CDebug
  LOG(ERROR)
      << "get last w, lp size:"
      << (lastw_.find(addr_idx) == lastw_.end() ? 0 : lastw_[addr_idx].size())
      << " address:" << address << " addr idx:" << addr_idx << " lr size:"
      << (lastr_.find(addr_idx) == lastr_.end() ? 0 : lastr_[addr_idx].size());
#endif
  // return -1;
  auto it = lastw_.find(addr_idx);
  if (it == lastw_.end() || it->second.empty()) {
    // LOG(ERROR)<<"no last:"<<address;
    if (ds_ == false || lastr_.find(addr_idx) == lastr_.end() ||
        lastr_[addr_idx].empty()) {
      return -1;
    }
    auto pit = lastr_[addr_idx].begin();
    if (*pit == idx) pit++;
    if (pit == lastr_[addr_idx].end()) {
      return -1;
    }
    return *pit;
    if (*lastr_[addr_idx].begin() == 0 || *lastr_[addr_idx].begin() == idx) {
      LOG(ERROR) << "get last w:" << *lastr_[addr_idx].begin()
                 << " size:" << lastr_[addr_idx].size();
    }
...
int FXController::GetLastWOnly(const uint256_t& address, int addr_idx) {
  return GetLastW(address, addr_idx, 0);
}

void FXController::Abort(int64_t commit_id) {
  if (aborted_[commit_id]) {
    return;
  }
...
void FXController::RecursiveAbort(int64_t idx, const uint256_t& addr) {
#ifdef CDebug
...
void FXController::AbortNodeFrom(int64_t idx, const uint256_t& addr) {
#ifdef CDebug
...
void FXController::AbortNode(int64_t idx) {
...
int FXController::GetDep(int from, int to) {
  LOG(ERROR) << "get dep:"
             << "fromm:" << from << " to:" << to;
  if (to == from) {
    return 0;
...
void FXController::ResetReach(int64_t commit_id) {
  reach_[commit_id].reset();
  reach_[commit_id].set(commit_id, true);
...
void FXController::RemoveNode(int64_t commit_id) {
#ifdef CDebug
  ...
  addr_changes_list_[commit_id].clear();
  has_write_[commit_id] = false;
  is_only_[commit_id] = false;

  // LOG(ERROR)<<"remove node done:"<<commit_id;
}

void FXController::SetOnly(int commit_id) { is_only_[commit_id] = true; }
bool FXController::Reach(int from, int to) {
  // LOG(ERROR)<<" reach from:"<<from<<" to:"<<to;
  assert(from != to);
...
std::set<int> FXController::GetReach(int from) {
  std::queue<int> q;
  q.push(from);
...
std::set<int> FXController::GetReach(int from, int to) {
  std::queue<int> q;
  q.push(from);
...
bool FXController::PrintReach(int from, int to) {
  // return reach_[to][from];
  std::queue<int> q;
  q.push(from);
...
bool FXController::ContainRead(int commit_id, const uint256_t& address) {
#ifdef CDebug
...
void FXController::ConnectSelf(int idx, int last_idx, const uint256_t& address,
                               int addr_idx) {
...
void FXController::AttachReadOnly(int commit_id, const uint256_t& address) {
  ...
  attach_[commit_id] = last_commit_;
#ifdef CDebug
  LOG(ERROR) << "attach :" << commit_id << " from address:" << address
             << " idx:" << last_commit_;
#endif
}
...
#endif
    if (idx != -1) {
      auto it = commit_list_.find(address);
      if (it != commit_list_.end()) {
        auto data_idx = it->second.upper_bound(idx);
        if (data_idx == it->second.begin()) {
        } else {
          --data_idx;
#ifdef CDebug
          LOG(ERROR) << "find adress :" << address
                     << " from:" << (*data_idx).first
                     << " commit id:" << commit_id;
#endif
          data.data = (*data_idx).second.first;
          data.version = (*data_idx).second.second;
          return;
        }
      }
    }
...
#endif
  commit_list_[address][commit_idx] = std::make_pair(data, version);
}

bool FXController::Connect(int idx, int last_idx, const uint256_t& address,
...
bool FXController::AddNewNode(const uint256_t& address, int addr_id,
                              int64_t commit_id, Data& data) {
#ifdef CDebug
  //  LOG(ERROR)<<"add new node:"<<commit_id;
#endif
  if (data.state == STORE) {
    int last_r = GetLastR(address, addr_id, commit_id);
    ...
    int last_w = GetLastW(address, addr_id, commit_id);
#ifdef CDebug
...
int FXController::FindW(int64_t commit_id, const uint256_t& address,
                        int addr_idx) {
  std::queue<int> q;
  q.push(commit_id);
...
int FXController::FindR(int64_t commit_id, const uint256_t& address,
                        int addr_idx) {
  std::queue<int> q;
  q.push(commit_id);
...
bool FXController::AddOldNode(const uint256_t& address, int addr_idx,
                              int64_t commit_id, Data& data) {
#ifdef CDebug
// LOG(ERROR)<<"add old node address:"<<address<<" commit id:"<<commit_id;
  ...
  if (change_set.empty()) {
...
#endif
    if (data.state == STORE) {
      int last_r = GetLastR(address, addr_idx, commit_id);
#ifdef CDebug
      LOG(ERROR) << "get last r:" << last_r << " commit :" << commit_id
                 << " address:" << address;
#endif
   ...
   :"<<last_r<<" fail"; return ret;
      ...
      int last_w = GetLastW(address, addr_idx, commit_id);
#ifdef CDebug
      LOG(ERROR) << "get last w:" << last_w << " commit :" << commit_id
                 << " address:" << address;
#endif
...
void FXController::AppendPreRecord(const uint256_t& address, int64_t commit_id,
                                   Data& data) {
  if (is_only_[commit_id]) {
    AttachReadOnly(commit_id, address);
    ReadReadOnly(commit_id, address, data);
    return;
  }
  ...
  bool ret = true;
  {
    ...
    LOG(ERROR) << "append address:" << address
               << " size:" << changes_list_[commit_id].size()
               << " commit id:" << commit_id << " type:" << data.state
...
bool FXController::CheckCommit(int64_t commit_id) {
...
bool FXController::CommitUpdates(int64_t commit_id) {
      ...
      for (int i = it.second.size() - 1; i >= 0 && !done; --i) {
        ...
        switch (op.state) {
          ...
          case STORE:
            ...
            // id:"<<commit_id; storage_->StoreWithVersion(it.first, op.data,
            // op.version, false);
            AddDataSnap(commit_idx_, it.first, op.data, op.version);
            done = true;
            break;
          case REMOVE:
...
bool FXController::Commit(int64_t commit_id) {
#ifdef CDebug
  LOG(ERROR) << "commit id:" << commit_id;
...
const std::vector<int64_t>& FXController::GetRedo() { return redo_; }
const std::vector<int64_t>& FXController::GetDone() { return done_; }
uint64_t FXController::GetDelay(int64_t commit_id) {
  std::unique_lock lock(g_mutex_);
