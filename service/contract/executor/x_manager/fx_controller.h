...
class FXController : public ConcurrencyController {
 ...
 private:
  ...
  int GetLastR(const uint256_t& address, int addr_id, int commit_id);
  int GetLastW(const uint256_t& address, int addr_id, int commit_id);
  ...
  void AddDataSnap(int commit_idx, const uint256_t& address,
 ...
 private:
  ...
  std::map<uint256_t, std::map<int, std::pair<uint256_t, int64_t>>>
      commit_list_;
  int last_commit_;
  int attach_[2048];
  int commit_idx_;
