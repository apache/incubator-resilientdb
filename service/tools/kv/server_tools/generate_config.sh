...
usage() {
  cat <<'USAGE'
Usage: ./service/tools/kv/server_tools/generate_config.sh [options]

Options:
  --replicas N    Number of replica nodes to configure. Default: 4
...
echo "Generating 3PC KV config: replicas=${REPLICA_NUM} clients=${CLIENT_NUM} port_base=${PORT_BASE}"

./service/tools/config/generate_config.sh \
  "${WORKSPACE}" \
  "${CERT_PATH}" \
  "${CERT_PATH}" \
  "${CONFIG_PATH}" \
