...
usage() {
  cat <<'USAGE'
Usage: ./service/tools/kv/server_tools/start_kv_service_sharded_3pc.sh [options] [-- bazel_args...]
...
Examples:
  ./service/tools/kv/server_tools/start_kv_service_sharded_3pc.sh --shards 4 --replicas-per-shard 4 --clients 1
  ./service/tools/kv/server_tools/start_kv_service_sharded_3pc.sh --shards 2 --replicas-per-shard 4 --clients 2 -- --define enable_leveldb=True
USAGE
}
...
killall -9 kv_service kv_service_3pc kv_service_sharded_3pc 2>/dev/null || true

echo "Cleaning old KV launch logs"
rm -f server[0-9]*.log client.log client[0-9]*.log

SERVER_PATH=./bazel-bin/service/kv/kv_service_sharded_3pc
CERT_PATH=$PWD/service/tools/data/cert

if [[ "${DO_BUILD}" -eq 1 ]]; then
  bazel build //service/kv:kv_service_sharded_3pc "${BAZEL_ARGS[@]}"
fi

echo "Launching sharded 3PC KV service: shards=${SHARD_NUM} replicas_per_shard=${REPLICAS_PER_SHARD} replicas=${REPLICA_NUM} clients=${CLIENT_NUM}"
