...
nohup sudo pyroscope exec -spy-name ebpfspy -application-name cpp_client_2 $SERVER_PATH $SERVER_CONFIG $CERT_PATH/node2.key.pri $CERT_PATH/cert_2.cert 8091 > server1.log &
