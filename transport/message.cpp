...
Message *Message::create_message(RemReqType rtype)
{
	...
	case PREP_CERTIFICATE_MSG:
		msg = new PrepCertificateMessage;
		break;
#endif
...
void Message::release_message(Message *msg)
{
	...
	case PREP_CERTIFICATE_MSG:
	{
		PrepCertificateMessage *m_msg = (PrepCertificateMessage *)msg;
		m_msg->release();
		delete m_msg;
		break;
	}
...
#if SGX
	COPY_VAL(tee_signature_size, buf, ptr);
	ptr = buf_to_string(buf, ptr, tee_signature, tee_signature_size);
#endif
...
#if SGX
	COPY_BUF(buf, tee_signature_size, ptr);
	for (uint j = 0; j < tee_signature.size(); j++)
	{
		v = tee_signature[j];
		COPY_BUF(buf, v, ptr);
	}
...
bool BatchRequests::validate(uint64_t thd_id)
{
	...
	// cout << "Verifying transaction: " << this->txn_id << endl;
	// fflush(stdout);
	try
	{
		string sim_msg = "CMAC Test";
		string sim_key = "B8B34DA2D4C4D578D8494390E3DFE7A7";
		string sim_cmac = "8C72D147FF9B25699B6898379AF44D8F";
		CmacVerifyString(sim_key, sim_msg, sim_cmac);
	}
	catch (const CryptoPP::Exception &e)
	{
	}

	// if(tee_verify_counter_and_sign(this->txn_id, this->tee_signature) != TEE_VERIFY){
	// 	assert(0);
	// 	return false;
	// }
#endif
