...
void ZZY::SendFail(const Transaction& txn) { fail_func_(txn); }
...
void ZZY::AsyncCommit() {
  while (!IsStop()) {
    ...
    commit_(*txn);
    {
      ...
      // LOG(ERROR)<<"receive commit:"<<" seq:"<<seq<<" txn
      // hash:"<<txn->hash().size();
    }
  }
}
...
bool ZZY::ReceiveTransaction(std::unique_ptr<Transaction> txn) {
  ...
  // LOG(ERROR)<<" get water mark:"<<water_mark<<" seq:"<<seq_<<" commit
  // :"<<commit_seq_;
  if (water_mark > 0) {
...
bool ZZY::Verify(const Proposal& proposal) {
  ...
  for (const auto& qc : proposal.qc().qc()) {
    ...
    bool valid = verifier_->VerifyMessage(data_hash, qc.sign());
    if (!valid) {
...
bool ZZY::ReceiveCommit(std::unique_ptr<Proposal> proposal) {
  ...
  // LOG(ERROR)<<" client commit seq:"<<seq<<" proposer:"<<proposer<<" hash
  // size:"<<hash.size()<<" local_id:"<<local_id;
