...
// ========================================================
std::unique_ptr<ConcurrencyController::ModifyMap > DXController::GetChangeList(int64_t commit_id){
  // read lock
...
void DXController::StoreInternal(const int64_t commit_id, const uint256_t& address, const uint256_t& value, int version) {
...
uint256_t DXController::LoadInternal(const int64_t commit_id, const uint256_t& address, int version) {
  if(aborted_[commit_id]){
    throw eevm::Exception(
        eevm::Exception::Type::outOfGas,
        "Get lock fail");
...
void DXController::Store(const int64_t commit_id, const uint256_t& address, const uint256_t& value, int version) {
  StoreInternal(commit_id, address, value, version);
}

uint256_t DXController::Load(const int64_t commit_id, const uint256_t& address, int version) {
  return LoadInternal(commit_id, address, version);
}

bool DXController::Remove(const int64_t commit_id, const uint256_t& key, int version) {
...
void DXController::AddDataToChangeList(std::vector<Data>& change_set, const Data& data){
  if(change_set.empty()){
...
void DXController::ConnectSelf(int idx, int last_idx, const uint256_t& address, int addr_idx){
...
bool DXController::Connect(int idx, int last_idx, const uint256_t& address, int addr_idx, bool is_read){
  int addr_idx_e = addr_idx&window_size_;
  if(last_idx==0){
    if(!is_read){
      ds_ = true;
...
int DXController::FindW(int64_t commit_id, const uint256_t& address, int addr_idx) {
...
int DXController::FindR(int64_t commit_id, const uint256_t& address, int addr_idx) {
...
void DXController::AppendPreRecord(const uint256_t& address, 
    int64_t commit_id, Data& data) {
...
const std::vector<int64_t> &DXController::GetRedo(){
  return redo_;
}
const std::vector<int64_t> &DXController::GetDone(){
  return done_;
}
