...
SequentialCCController::SequentialCCController(DataStorage * storage) : ConcurrencyController(storage){
...
std::vector<int64_t>& SequentialCCController::GetRedo(){
  return redo_;
}
...
void SequentialCCController::PushCommit(int64_t commit_id, const ModifyMap& local_changes) {
...
bool SequentialCCController::CheckFirstCommit(const uint256_t& address, int64_t commit_id){
...
bool SequentialCCController::IsRead(const uint256_t& address, int64_t commit_id){
  const auto& change_set = changes_list_[commit_id];
  if(change_set.empty()){
    LOG(ERROR)<<" no commit id record found:"<<commit_id;
  ...
  for(const auto& it : change_set){
    ...
    for(auto& op : it.second){
      if(op.state == LOAD){
        ...
        if(op.version != v){
          //LOG(ERROR)<<"check load:"<<address<<" version:"<<v<<" data v:"<<op.version<<" commit id:"<<commit_id<<" mlist:"<<m_list_[it.first];
...
const ConcurrencyController::ModifyMap * SequentialCCController::GetChangeList(int64_t commit_id){
  // read lock
...
int64_t SequentialCCController::Remove(const uint256_t& address, int64_t commit_id, uint64_t v){
      int idx = GetHashKey(address);
