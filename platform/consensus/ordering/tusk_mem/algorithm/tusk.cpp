...
void Tusk::AsyncSend() {
  ...
  while (!IsStop()) {
    auto txn = txns_.Pop();
    if (txn == nullptr) {
      LOG(ERROR)<<"not enough client req";
      if(start_){
        //break;
      }
      continue;
...
void Tusk::CommitProposal(int round, int proposer) {
  ...
  if(p==nullptr){
    ...
    future_txn_.insert(std::make_pair(round, proposer));
    //assert(1==0);
    return;
  }
  assert(p !=nullptr);
...
bool Tusk::ReceiveBlock(std::unique_ptr<Proposal> proposal) {
  ...
  //           << " round:" << proposal->header().round();
    ...
    std::unique_lock<std::mutex> lk(check_block_mutex_);
    if(future_txn_.find(std::make_pair(round, proposer)) != future_txn_.end()){
   //   LOG(ERROR)<<" commit future block";
       proposal_manager_->AddBlock(std::move(proposal));
      CommitProposal(round, proposer);
      return true;
    }
...
void Tusk::ReceiveBlockCert(std::unique_ptr<Certificate> cert) {
    ...
    //LOG(ERROR)<<" receive cert from :"<<proposer<<" round:"<<round;
