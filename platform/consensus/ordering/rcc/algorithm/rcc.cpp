    ...
    while (!IsStop()) {
      if (seq_set_[next_seq_].size() == totoal_proposer_num_) {
        ...
        for (auto& it : seq_set_[next_seq_]) {
          ...
          // proposer:"<<it.second->header().proposer_id()<<" commit
          // wait:"<<commit_time - it.second->queuing_time();
          ...
          global_stats_->AddCommitWaitingLatency(commit_time -
                                                 it.second->queuing_time());
        }
        // LOG(ERROR)<<" commit seq:"<<next_seq_<<" transac num:"<<num<<" block
        // num:"<<pro;
        ...
        global_stats_->AddCommitBlock(pro);
        {
          ...
          commited_seq_ =
              std::max(static_cast<int64_t>(next_seq_), commited_seq_);
...
void RCC::CommitProposal(std::unique_ptr<Proposal> p) {
  {
    ...
    // commited_seq_); vote_cv_.notify_all();
    global_stats_->AddCommitLatency(GetCurrentTime() -
                                    p->header().create_time());
    // LOG(ERROR)<<" seq:"<<p->header().seq()<<"
    // proposer:"<<p->header().proposer_id()<<" commit:"<<GetCurrentTime()<<"
    // create time:"<<p->header().create_time()<<" commit
    // time:"<<(GetCurrentTime() - p->header().create_time());
    // global_stats_->AddCommitWaitingLatency(GetCurrentTime() -
    // last_commit_time_); last_commit_time_ = GetCurrentTime();
  }
...
void RCC::AsyncSend() {
  ...
  while (!IsStop()) {
    ...
    while (!IsStop()) {
      ...
      // LOG(ERROR)<<" current seq:"<<proposal_manager_->CurrentSeq()<<" commit
      // seq:"<<commited_seq_;
      if (proposal_manager_->CurrentSeq() - commited_seq_ <= limit) {
        break;
      }
    }
    // LOG(ERROR)<<" committed seq:"<<commited_seq_<<"
    // current:"<<proposal_manager_->CurrentSeq()<<" quueing
    // time:"<<GetCurrentTime() - txn->create_time()<<" queue
    // size:"<<queue_size_;
    ...
    // LOG(ERROR)<<" propose txn size:"<<txns.size()<<"
    // batch_size_:"<<batch_size_<<" queue size:"<<queue_size_;
