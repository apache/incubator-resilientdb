...
Pbft::Pbft(const ResDBConfig& config, int id, int f, int total_num,
...
void Pbft::SendFail(const Transaction& txn) { fail_func_(txn); }
...
bool Pbft::ReceivePrepare(std::unique_ptr<Proposal> proposal) {
  ...
  // LOG(ERROR)<<"recv prepare from:"<<proposal->proposer()<<"
  // id:"<<proposal->seq(); assert(proposer != 2); assert(proposer != 3);
  std::unique_ptr<Transaction> txn = nullptr;
  bool done = false;
  {
...
bool Pbft::ReceiveCommit(std::unique_ptr<Proposal> proposal) {
  ...
  // num:"<<commit_received_[seq%1].size()<<" seq:"<<seq; assert(proposer != 2);
  // assert(proposer != 3);
