...
ProposalManager::ProposalManager(int32_t id, int limit_count, int slot_num, SignatureVerifier* verifier, int total_num, int fork_tail_num)
    : id_(id), limit_count_(limit_count), slot_num_(slot_num), verifier_(verifier), total_num_(total_num), fork_tail_num_(fork_tail_num) {
    round_ = 1;
...
void ProposalManager::AddQC(std::unique_ptr<QC> qc){
  ...
  if(generic_qc_.view() == 0 || generic_qc_.view() < qc->view() || (generic_qc_.view() == qc->view() && generic_qc_.slot() < qc->slot())){
    ...
    // else {
    //   LOG(ERROR) << "Here " << generic_qc_.view() << " " << generic_qc_.slot();
    // }
    if(qc->is_final()){
      round_ = qc->view()+1;
    }
