...
/*
int Cassandra::ReceiveRecovery(const CommittedProposals& proposals) {
  std::unique_lock<std::mutex> lk(mutex_);
  for(const Proposal& p : proposals.proposals()){
    if(graph_->RecoveryCommit(p)){
      CommitProposal(p);
    }
  }
  return 0;
...
bool Cassandra::CheckState(MessageType type, ProposalState state){
  switch(state) {
    case ProposalState::New:
      return type >= MessageType::Vote;
    case ProposalState::Prepared:
      return type >= MessageType::Prepare;
     default:
      return false;
  }
}

void Cassandra::TrySendRecoveery(const Proposal& proposal) {
  std::vector<Proposal> proposals = graph_->GetCommittedProposalFrom(proposal);
  if(proposals.size()==0){
  return;
  }
  CommittedProposals msg;
  for(const Proposal& p : proposals){
    *msg.add_proposals() = p;
  }
  msg.set_sender_id(id_);
  //LOG(ERROR)<<"send recovery to:"<<proposal.proposal_id()<<"
proposal:"<<proposal.proposer_id()<<" id:"<<proposal.proposal_id()<<" proposal
size:"<<proposals.size(); single_call_(MessageType::Recovery, msg,
proposal.header().proposer_id());
}
