#include "platform/consensus/ordering/cassandra/algorithm/proposal_manager_mem.h"
...
ProposalManagerMem::ProposalManagerMem(int32_t id, ProposalGraph* graph)
    : id_(id), graph_(graph) {
  local_proposal_id_ = 1;
  miner_ = std::make_unique<Miner>();
}

std::unique_ptr<Proposal> ProposalManagerMem::GenerateProposal(
...
bool ProposalManagerMem::VerifyProposal(const Proposal& proposal) {
