...
ProposalGraph::ProposalGraph(int fault_num, int total_num)
    : f_(fault_num), total_num_(total_num) {
...
int ProposalGraph::GetLeader(const std::string& hash) {
  auto node_it = node_info_.find(hash);
  if (node_it == node_info_.end()) {
    LOG(ERROR) << "no hash:" << hash.size();
    return -1;
  }

  return (node_it->second->proposal.header().proposer_id() + 1) % total_num_;
}

void ProposalGraph::CheckHistory(const Proposal& proposal) {
  // LOG(ERROR)<<"check proposal history:"<<proposal.history_size();
  assert(proposal.history_size() >= 0);
  if (proposal.history_size() <= 0) {
    return;
  }
  int leader = 1;
  if (proposal.history_size() == 1) {
    leader = 1;
  } else {
    leader = GetLeader(proposal.history(1).hash());
  }
  // LOG(ERROR)<<"proposal get leader:"<<leader;
  if (leader < 0) {
    return;
  }

  std::string hash = proposal.history(0).hash();
  auto node_it = node_info_.find(hash);
  if (node_it->second->proposal.header().proposer_id() != leader) {
    LOG(ERROR) << "leader not match:" << leader << " proposer:"
               << node_it->second->proposal.header().proposer_id();
    return;
  }

  LOG(ERROR) << "vote num:" << node_it->second->votes[ProposalState::New].size()
             << " total num:" << total_num_;
  if (node_it->second->votes[ProposalState::New].size() == total_num_) {
    node_it->second->state = ProposalState::PreCommit;
    Commit(hash);
  } else {
    CheckState(proposal);
  }
...
void ProposalGraph::CheckState(const Proposal& proposal) {
  for (const auto& history : proposal.history()) {
    std::string hash = history.hash();
    auto node_it = node_info_.find(hash);
    // LOG(ERROR)<<"check
...
void ProposalGraph::Confirm(int height, int leader) {
  for (auto& it : new_proposals_) {
    const Proposal* p = it.second;
    assert(p->header().height() == height);
    if (p->header().proposer_id() == leader) {
      auto node_it = node_info_.find(it.first);
      assert(node_it != node_info_.end());
      assert(node_it->second->state == ProposalState::New);
      node_it->second->state = ProposalState::Prepared;
      break;
    }
  }
  LOG(ERROR) << "height:" << height << " leader not found:" << leader;
  assert(1 == 0);
}
  ...
  auto bfs = [&]() {
    ...
    while (!q.empty()) {
      ...
      if (it->second->state == ProposalState::Committed) {
        ...
        // sender:"<<p->header().proposer_id()<<" proposal
        // id:"<<p->header().proposal_id()<<" has been committed";
        continue;
      }
...
void ProposalGraph::UpdateHistory(Proposal* proposal) {
  ...
  // LOG(ERROR)<<"get history:"<<proposal->history_size();
}
...
Proposal* ProposalGraph::GetStrongestProposal() {
  ...
  int next_leader = GetLeader(latest_sp_);
  if (next_leader == -1) {
    // LOG(ERROR)<<"current_height_:"<<current_height_<<" next
    // leader:"<<next_leader;
    assert(current_height_ == 1);
    next_leader = 1;
  }
  NodeInfo* sp = nullptr;
  for (const auto& last_hash : last_node_[current_height_]) {
    NodeInfo* node_info = node_info_[last_hash].get();
    if (node_info->proposal.header().proposer_id() == next_leader) {
      sp = node_info;
      break;
    }
  ...
  //          leader:"<<next_leader;
  latest_sp_ = sp->proposal.header().hash();
  return &sp->proposal;
}
