...
class Cassandra {
 public:
  ...
  bool ReceiveProposalResponse(const ProposalResponse& response);
 ...
 private:
  ...
  void SendProposalResponse(const Proposal& proposal);
 ...
 private:
  ...
  std::map<int, std::set<int>> received_num_, received_leader_, received_;
  std::map<int, int64_t> received_time_;
  // int state_;
