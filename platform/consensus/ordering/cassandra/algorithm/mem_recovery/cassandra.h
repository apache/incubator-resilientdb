...
#include "platform/consensus/ordering/cassandra/algorithm/mem_recovery/proposal_graph.h"
#include "platform/consensus/ordering/cassandra/algorithm/mem_recovery/proposal_manager.h"
...
class Cassandra {
 public:
  Cassandra(int id, int batch_size, int total_num, int f,
            ...
            std::function<int(const Transaction& txn)> prepare = nullptr);
  ~Cassandra();
 ...
 private:
  ...
  std::function<int(int, const google::protobuf::Message& msg, int)>
      single_call_;
  std::function<int(const Transaction&)> commit_, prepare_;
