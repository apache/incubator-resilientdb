...
int Consensus::ProcessCustomConsensus(std::unique_ptr<Request> request) {
  ...
  std::string txn_id = request->hash();

  switch (msg_type) {
    case TwoPC::PREPARE:
      return twopc_->ProcessPrepare(txn_id, *request);
    case TwoPC::VOTE: {
      bool vote = (request->seq() == 1);
      return twopc_->ProcessVote(txn_id, request->sender_id(), vote);
    }
    case TwoPC::COMMIT:
...
int Consensus::ProcessNewTransaction(std::unique_ptr<Request> request) {
  if (config_.GetSelfInfo().id() == coordinator_id_) {
    std::string txn_id = "txn_" + std::to_string(request->seq()) + "_" +
