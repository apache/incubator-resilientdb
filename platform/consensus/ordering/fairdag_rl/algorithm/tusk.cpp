...
void Tusk::Stop() { is_stop_ = true; }

bool Tusk::IsStop() { return is_stop_; }
...
void Tusk::AsyncExecute() {
  while (!IsStop()) {
    ...
    while (!q.empty()) {
      ...
      q.pop();
      // LOG(ERROR)<<"check proposal round:"<<p->header().round()<<"
    ...
    int num = 0;
    int pro = 0;
    std::unique_ptr<std::vector<std::unique_ptr<Transaction>>> txns =
    ...
    for (auto& it : ps) {
      for (auto& p : it.second) {
        ...
        // global_stats_->AddCommitLatency(commit_time -
        // p->header().create_time() - waiting_time);
        ...
        for (TxnDigest& dg : *p->mutable_digest()) {
          ...
          proposal_manager_->FetchTxnCopy(dg.hash()); if(txn == nullptr){
            LOG(ERROR)<<"no txn from:"<<dg.proposer()<<" user seq:"<<dg.seq()<<"
          from "<<p->header().proposer_id();
          }
          assert(txn != nullptr);
          */
          ...
          // global_stats_->AddCommitDelay(commit_time- txn->create_time() -
          // txn->queuing_time());

          v.insert(std::make_pair(dg.proposer(), dg.seq()));
          // LOG(ERROR)<<"commit from:"<<dg.proposer()<<" user
          // seq:"<<dg.seq()<<" ts:"<<dg.ts()<<" txn
          // proposer:"<<txn->proposer()<<" delay:"<<(GetCurrentTime()-
          // txn->create_time() - txn->queuing_time())<<" queuing
          // time:"<<txn->queuing_time()<<" proposer create
          // time:"<<p->header().create_time()<<" txn create
          // time:"<<txn->create_time() <<" queuing time:"<<txn->queuing_time();
    ...
    // LOG(ERROR)<<"commit proposal:"<<txns.size()<<" round:"<<commit_round<<"
    // blocks:"<<pro<<" txn size:"<< num<<" uniq:"<<v.size(); commit_(*txns);
...
bool Tusk::SendBlockAck(std::unique_ptr<Proposal> proposal) {
  ...
  *metadata.mutable_sign() = *hash_signature_or;
  ...
  {
    // std::unique_lock<std::mutex> lk(txn_mutex_);
    // LOG(ERROR) << "send back ack to block round:" <<
    // proposal->header().round()<<" from " << proposal->header().proposer_id();
