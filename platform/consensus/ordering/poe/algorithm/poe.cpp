...
int Next(int p) { return 1; }

PoE::PoE(const ResDBConfig& config, int id, int f, int total_num,
...
void PoE::SendFail(const Transaction& txn) { fail_func_(txn); }
...
bool PoE::ReceiveSupport(std::unique_ptr<Proposal> proposal) {
  ...
  {
    // LOG(ERROR)<<"recv proposal support from:"<<proposal->proposer()<<"
    // id:"<<proposal->seq();
...
bool PoE::ReceiveCert(std::unique_ptr<Proposal> proposal) {
  ...
  int64_t seq = proposal->seq();
  int proposer = proposal->proposer();
  // LOG(ERROR)<<"recv proposal cert from:"<<proposal->proposer()<<"
  // id:"<<proposal->seq();
  Verify(*proposal);
  if (true) {
...
bool PoE::Verify(const Proposal& proposal) {
  for (const auto& s : proposal.sign()) {
    bool valid = verifier_->VerifyMessage(proposal.hash(), s);
    assert(valid);
  }
  return true;
}
...
bool PoE::ReceivePrepare(std::unique_ptr<Proposal> proposal) {
  ...
  bool done = false;
  {
    // LOG(ERROR)<<"recv proposal from:"<<proposal->proposer()<<"
    // id:"<<proposal->seq();
...
bool PoE::ReceiveCommit(std::unique_ptr<Proposal> proposal) {
  ...
  // LOG(ERROR)<<"receive commit:"<<proposer<<"
  // num:"<<commit_received_[seq%1][seq].size()<<" seq:"<<seq;
