  ...
  while (!is_stop_) {
    ...
    for (const Block& block : p->sub_block()) {
      ...
      std::unique_ptr<Block> data_block =
          proposal_manager_->GetBlock(block.hash(), block.sender_id());
      ...
      //<<"local id:"<<block.local_id();
      auto it = committed.find(std::make_pair(block.sender_id(), block.local_id()));
      if( it != committed.end()){
        //LOG(ERROR)<<"!!!!!!!!! proposal from:" <<p->header().proposer_id()
         // <<"local id:"<<block.local_id()
          //<<" has been committed";
      ...
      committed.insert(std::make_pair(block.sender_id(), block.local_id()));
  ...
  while (!IsStop()) {
    ...
    */
    ...
    for(int i = 1; i < batch_size_; ++i){
      std::unique_ptr<Transaction> txn = txns_.Pop(1000);
      if(txn == nullptr){
        break;
      }
      //LOG(ERROR)<<"get txn, proxy id:"<<txn->proxy_id()<<" hash:"<<txn->hash();
    ...
    //LOG(ERROR)<<" send block:"<<block->local_id()<<" txn size:"<<txns.size();
