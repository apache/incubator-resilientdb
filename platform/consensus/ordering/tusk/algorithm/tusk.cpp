...
Tusk::Tusk(int id, int f, int total_num, SignatureVerifier* verifier, const ResDBConfig& config)
    ...
    for (int r = previous_round + 2; r <= round; r += 2) {
      ...
      while(!IsStop()){
        ...
        if (req == nullptr) {
          ...
          // Mirrors sync_fides/fides pattern; lets commit advance past a
          // crashed anchor instead of blocking forever on GetRequest.
      ...
      if (req == nullptr) continue;  // Dead leader, skip this round
...
void Tusk::CommitProposal(int round, int proposer) {
  ...
  if(p==nullptr){
    // Already consumed by a previous AsyncExecute BFS (our FetchRequest
    // returns null when the block has been moved out). Not fatal —
    // just skip this duplicate commit.
    return;
  }
...
void Tusk::ReceiveBlockACK(std::unique_ptr<Metadata> metadata) {
  ...
  if (received_num_[hash].size() == limit_count_) {
    ...
    if (p == nullptr || p->header().proposer_id() != id_ ||
        p->header().round() != round) {
      // Local block was evicted or the ack is stale — skip instead of
      // crashing the replica during the crash-fault transition.
      LOG(ERROR) << "Tusk: stale ack for round " << round
                 << ", dropping";
      return;
    }
...
void Tusk::ReceiveBlockCert(std::unique_ptr<Certificate> cert) {
  int64_t start_time = GetCurrentTime();
  if(!VerifyCert(*cert)){
    LOG(ERROR) << "Tusk: cert verification failed, dropping";
    return;
  }
