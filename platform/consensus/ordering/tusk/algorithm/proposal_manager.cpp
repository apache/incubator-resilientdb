...
std::unique_ptr<Proposal> ProposalManager::FetchRequest(int round, int sender) {
  ...
  if (bit == block_.end() || bit->second == nullptr) {
    // Defensive: the block may already have been consumed by a previous
    // cascade commit (FetchRequest is destructive). Return nullptr so
    // CommitProposal can skip instead of crashing the replica.
    return nullptr;
  }
  auto tmp = std::move(bit->second);
...
void ProposalManager::GetMetaData(Proposal* proposal) {
  ...
  for (const auto& meta : latest_cert_from_sender_) {
    ...
    if (meta.second->round() >= round_) {
      ...
      // Defensive: a race between AddCert and GetMetaData could leave
      // meta.second->round() slightly ahead of round_. Not safety-
      // critical — just add the cert.
      *proposal->mutable_header()->mutable_weak_cert()->add_cert() =
          *meta.second;
    }
  }
