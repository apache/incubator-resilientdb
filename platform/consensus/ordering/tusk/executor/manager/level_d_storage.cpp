...
std::string GetString(const uint256_t& key){
  return "";
}

uint256_t GetInt256(const std::string& value){
  return 0;
}
...
uint256_t GetValue(const std::string& value){
  return GetInt256(std::string(value.c_str()+sizeof(int64_t),value.size()-sizeof(int64_t)));
}
