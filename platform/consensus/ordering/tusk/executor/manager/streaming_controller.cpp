...
std::vector<int64_t>& StreamingController::GetRedo(){
  return redo_;
}
...
void StreamingController::PushCommit(int64_t commit_id, const ModifyMap& local_changes) {
...
bool StreamingController::CheckFirstCommit(const uint256_t& address, int64_t commit_id){
...
int64_t StreamingController::Remove(const uint256_t& address, int64_t commit_id, uint64_t v){
  int idx = GetHashKey(address);
  if(commit_list_[idx].find(address) == commit_list_[idx].end()){
