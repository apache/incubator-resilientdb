...
XController::XController(DataStorage* storage)
...
void XController::UseOCC() { use_occ_ = true; }
...
void XController::SetCallback(CallBack call_back) { call_back_ = call_back; }
...
const ConcurrencyController::ModifyMap* XController::GetChangeList(
    int64_t commit_id) {
  // read lock
...
/*
int64_t XController::Remove(const uint256_t& address, int64_t commit_id,
uint64_t v){ int idx = GetHashKey(address);
  //std::unique_lock lock(mutex_);
