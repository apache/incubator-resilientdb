...
namespace {
    ...
    void AppendRecord(const uint256_t& address, 
        int64_t commit_id, StreamingEController::CommitList * commit_list) {
    ...
    int64_t RemoveRecord(
        const uint256_t& address, int64_t commit_id, StreamingEController::CommitList * commit_list) {
  ...
  //LOG(ERROR)<<"append record addr:"<<address<<" commit id:"<<commit_id;
  {
    ...
    if(it == commit_set.end() || it->second.commit_version != version){
      ...
      if(it == commit_set.begin()){
        ...
        //LOG(ERROR)<<"get from self:"<<it->first<<" ver:"<<it->second.version<<" commit ver:"<<it->second.commit_version;
    }
    ...
    //LOG(ERROR)<<"!!!!! append id:"<<commit_id<<" state:"<<data.state<<" value:"<<data.data<<" ver:"<<data.version<<" addr:"<<address<<" commit version:"<<data.commit_version;
...
void StreamingEController::AppendPreRecord(const uint256_t& address, int hash_idx, 
...
int64_t StreamingEController::RemovePrecommitRecord(
    const uint256_t& address, int64_t commit_id) {
  int idx = GetHashKey(address);
...
bool StreamingEController::CheckFirstFromCommit(const uint256_t& address, int64_t commit_id){
  int idx = GetHashKey(address);
  ...
  if(commit_set.empty()){
    //LOG(ERROR)<<"commit set is empty:"<<commit_id<<" is pre:0 addr:"<<address;
  }
  assert(!commit_set.empty());

  if(commit_set.front() < commit_id){
    // not the first candidate
    //LOG(ERROR)<<"still have dep. commit id:"<<commit_id<<" dep:"<<commit_set.front()<<" addr:"<<address;
    return false;
  }
  if(commit_set.front() > commit_id){
    //LOG(ERROR)<<"set header:"<<commit_set.front()<<" current:"<<commit_id<<" is pre: 0";
    return false;
  }
...
bool StreamingEController::CheckFirstFromPreCommit(const uint256_t& address, int64_t commit_id){
  ...
  const auto& commit_set  = it->second;
  if(commit_set.empty()){
    //LOG(ERROR)<<"commit set is empty:"<<commit_id<<" is pre: 1 addr:"<<address;
  }
  assert(!commit_set.empty());

  if(commit_set.begin()->first < commit_id){
    // not the first candidate
    //LOG(ERROR)<<"still have dep. commit id:"<<commit_id<<" dep:"<<commit_set.begin()->first<<" address:"<<address;
    return false;
  }
  if(commit_set.begin()->first > commit_id){
    //LOG(ERROR)<<"set header:"<<commit_set.begin()->first<<" current:"<<commit_id<<" is pre: 1";
  }
    ...
    for(const auto& it : change_set){
      for(auto& op : it.second){
        ...
        if(op.state == LOAD){
          ...
          //LOG(ERROR)<<"op log:"<<op.version<<" db ver:"<<v<<" is pre:"<<is_pre<<" address:"<<it.first<<" version:"<<op.commit_version;
          if(op.version != v){
            //LOG(ERROR)<<"op log:"<<op.version<<" db ver:"<<v<<" is pre:"<<true<<" address:"<<it.first<<" check fail"<<" commit ver:"<<op.commit_version;
            if(is_pre==0){
              assert(1==0);
            }
...
void StreamingEController::RollBackData(const uint256_t& address, const Data& data){
...
bool StreamingEController::Commit(int64_t commit_id){
  ...
  //LOG(ERROR)<<"======================     commit id:"<<commit_id<<" last_pre_commit_id_:"<<last_pre_commit_id_;
