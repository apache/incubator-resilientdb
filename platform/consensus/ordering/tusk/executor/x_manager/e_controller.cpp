    ...
    if(data.state == LOAD){
      if(commit_set.empty()){
          ...
          if(cache_it == cache_data_.end()){
            ...
            //LOG(ERROR)<<"get from db commit id:"<<commit_id<<" address:"<<address;
          }
          else {
            LOG(ERROR)<<"get from cache commit id:"<<commit_id<<" address:"<<address;
...
bool EController::CommitUpdates(int64_t commit_id){
        ...
        case STORE:
        {
          if(op.version==0){
            //storage_->Store(it.first, op.data);
              ...
              tmp_data.push_back(std::make_pair(it.first, std::make_pair(op.data, 0)));
              //LOG(ERROR)<<"save to cache commit id:"<<commit_id<<" address:"<<it.first;
          }
          else {
            ...
            if(c_it == cache_data_.end()){
              uint64_t v = storage_->GetVersion(it.first, false);
              if(op.version >v){
                //cache_data_[it.first] = std::make_pair(op.data, op.version);
                //tmp_data.push_back(std::make_pair(it.first, std::make_pair(op.data, op.version)));
  ...
  for(auto& v : tmp_data){
    if(v.second.second==0){
        ...
        storage_->StoreWithVersion(v.first, v.second.first, v.second.second, false);
    }
  }
