...
// ========================================================
ConcurrencyController::ModifyMap * TwoPLController::GetChangeList(int64_t commit_id){
  // read lock
  return &(changes_list_[commit_id]);
}

bool TwoPLController::TryLock(const uint256_t& address, int64_t owner, LockType type){
  if(aborted_[owner]){
    return false;
  }
  return true;
...
void TwoPLController::StoreInternal(const int64_t commit_id, const uint256_t& address, const uint256_t& value, int version) {
...
uint256_t TwoPLController::LoadInternal(const int64_t commit_id, const uint256_t& address, int version) {
...
void TwoPLController::Store(const int64_t commit_id, const uint256_t& address, const uint256_t& value, int version) {
  StoreInternal(commit_id, address, value, version);
}

uint256_t TwoPLController::Load(const int64_t commit_id, const uint256_t& address, int version) {
  return LoadInternal(commit_id, address, version);
}

bool TwoPLController::Remove(const int64_t commit_id, const uint256_t& key, int version) {
...
void TwoPLController::AppendPreRecord(const uint256_t& address, 
    ...
    //LOG(ERROR)<<"append address:"<<address<<" commit id:"<<commit_id<<" state:"<<data.state;
  {
    std::unique_lock lock(g_mutex_);
    auto it = lock_table_.find(address);
    if(it != lock_table_.end() && it->second != commit_id){
     // LOG(ERROR)<<"append address:"<<address<<" commit id:"<<commit_id<<" state:"<<data.state<<" has been locked";
      Abort(commit_id);
      return;
    }
...
#ifdef CDebug
      LOG(ERROR)<<"LOAD from db:"<<" address:"<<address<<" commit id:"<<commit_id<<" version:"<<data.version;
  #endif
    }
  }
...
bool TwoPLController::CommitUpdates(int64_t commit_id){
  #ifdef CDebug
  LOG(ERROR)<<"check commit:"<<commit_id<<" abort:"<<aborted_[commit_id];
  #endif
...
const std::vector<int64_t> &TwoPLController::GetRedo(){
  return redo_;
}
const std::vector<int64_t> &TwoPLController::GetDone(){
  return done_;
}
