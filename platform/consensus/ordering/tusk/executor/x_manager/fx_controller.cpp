...
// ========================================================
std::unique_ptr<ConcurrencyController::ModifyMap > FXController::GetChangeList(int64_t commit_id){
  // read lock
...
void FXController::StoreInternal(const int64_t commit_id, const uint256_t& address, const uint256_t& value, int version) {
...
uint256_t FXController::LoadInternal(const int64_t commit_id, const uint256_t& address, int version) {
  if(aborted_[commit_id]){
    throw eevm::Exception(
        eevm::Exception::Type::outOfGas,
        "Get lock fail");
...
void FXController::Store(const int64_t commit_id, const uint256_t& address, const uint256_t& value, int version) {
  StoreInternal(commit_id, address, value, version);
}

uint256_t FXController::Load(const int64_t commit_id, const uint256_t& address, int version) {
  return LoadInternal(commit_id, address, version);
}

bool FXController::Remove(const int64_t commit_id, const uint256_t& key, int version) {
...
void FXController::AddDataToChangeList(std::vector<Data>& change_set, const Data& data){
  if(change_set.empty()){
...
int FXController::GetLastR(const uint256_t& address, int addr_idx){
    #ifdef CDebug
...
int FXController::GetLastW(const uint256_t& address, int addr_idx){
#ifdef CDebug
...
void FXController::SetOnly(int commit_id){
  is_only_[commit_id] = true;
}
...
void FXController::ConnectSelf(int idx, int last_idx, const uint256_t& address, int addr_idx){
...
#ifdef CDebug
  LOG(ERROR)<<"attach :"<<commit_id<<" from address:"<< address<<" idx:"<<last_commit_;
#endif
}

void FXController::ReadReadOnly(int commit_id, const uint256_t& address, Data &data){
  {
    std::unique_lock lock(s_mutex_);
    int idx = attach_[commit_id];
#ifdef CDebug
    LOG(ERROR)<<"read snap:"<<idx<<" commit id:"<<commit_id<<" address:"<<address;
#endif
    if(idx != -1){
      ...
      if(it != commit_list_.end()){
        ...
        if(data_idx == it->second.begin()){
          ...
          --data_idx;
#ifdef CDebug
          LOG(ERROR)<<"find adress :"<<address<<" from:"<<(*data_idx).first<<" commit id:"<<commit_id;
#endif
...
void FXController::AddDataSnap(int commit_idx, const uint256_t& address, const uint256_t& data, int64_t version) {
      std::unique_lock lock(s_mutex_);
#ifdef CDebug
...
bool FXController::Connect(int idx, int last_idx, const uint256_t& address, int addr_idx, bool is_read){
  int addr_idx_e = addr_idx&window_size_;
  if(last_idx==0){
    if(!is_read){
      ds_ = true;
...
int FXController::FindW(int64_t commit_id, const uint256_t& address, int addr_idx) {
...
int FXController::FindR(int64_t commit_id, const uint256_t& address, int addr_idx) {
...
void FXController::AppendPreRecord(const uint256_t& address, 
    int64_t commit_id, Data& data) {
...
const std::vector<int64_t> &FXController::GetRedo(){
  return redo_;
}
const std::vector<int64_t> &FXController::GetDone(){
  return done_;
}
