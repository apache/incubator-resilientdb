...
// Causes followers and candidates to start an election if no heartbeat
// received.
void LeaderElectionManager::MonitoringElectionTimeout() {
  while (!stop_.load()) {
    ...
    std::chrono::steady_clock::time_point wait_end_time_ =
        std::chrono::steady_clock::now();
    std::chrono::steady_clock::duration delta =
        wait_end_time_ - wait_start_time_;
    auto ms =
        std::chrono::duration_cast<std::chrono::milliseconds>(delta).count();
    if (raft_->liveness_logging_flag_) {
      LOG(INFO) << __FUNCTION__ << ": " << (leader ? "Leader" : "") << "Wait "
                << ms << "ms";
    }
    if (res == Waited::STOPPED) {
      ...
      if (raft_->liveness_logging_flag_) {
        LOG(INFO) << __FUNCTION__
                  << ": AE Broadcast reset leader heartbeat timer";
      }
      continue;
    }
