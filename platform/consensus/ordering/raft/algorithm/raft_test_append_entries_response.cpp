...
// Test 1: A leader receiving an AppendEntriesResponse success and updating the follower's matchIndex.
TEST_F(RaftTest, LeaderReceivesAppendEntriesResponseSuccess) {
  ...
  bool success = raft_->ReceiveAppendEntriesResponse(std::make_unique<AppendEntriesResponse>(aeResponse));
  EXPECT_TRUE(success);
  EXPECT_THAT(raft_->GetMatchIndex(), ::testing::ElementsAre(0, 2, 2, 0, 0));
}

// Test 2: A leader receiving an AppendEntriesResponse from a follower that in a newer term.
...
// Test 3: A leader receiving an AppendEntriesResponse success, updating the follower's matchIndex, and committing a new entry
...
// Test 4: A leader receiving an AppendEntriesResponse success and catching up a follower that is behind
...
// Test 5: A leader receiving an AppendEntriesResponse Failure and catching up a follower that is behind
