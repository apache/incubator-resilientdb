...
Thunderbolt::Thunderbolt(int id, int f, int total_num, SignatureVerifier* verifier)
  ...
  future_thread_ = std::thread(&Thunderbolt::RunFutureBlock, this);
  future_cert_thread_ = std::thread(&Thunderbolt::RunFutureCert, this);
  // block_thread_ = std::thread(&Thunderbolt::AsyncBlock, this);
}
...
void Thunderbolt::StopDone() { SwitchDAG(); }

void Thunderbolt::CreateManager(int dag) {
...
void Thunderbolt::NotifyInput() {
  txn_pending_num_ = 0;
  send_cv_.notify_all();
}

void Thunderbolt::WaitBatchReady() {
  while (!IsStop()) {
    {
...
bool Thunderbolt::WaitForNext(bool is_cross) {
  while (!IsStop()) {
...
void Thunderbolt::AddCrossTxn(int dag_id, int round, int proposer) {
  std::unique_lock<std::mutex> lk(cross_mutex_);
  // LOG(ERROR)<<" add cross txn round:"<<round<<" proposer:"<<proposer;
  future_cross_txn_[dag_id].insert(std::make_pair(round, proposer));
}

void Thunderbolt::RelaseCrossTxn(int dag_id, int round, int proposer) {
  std::unique_lock<std::mutex> lk(cross_mutex_);
...
ProposalManager* Thunderbolt::GetManager(int dag) { return proposal_manager_.get(); }

void Thunderbolt::SwitchDAG() {
...
void Thunderbolt::SendTransactions(
...
bool Thunderbolt::HasCheck(int round, int proposer) {
  return has_check_.find(std::make_pair(round, proposer)) != has_check_.end();
}

void Thunderbolt::SetCheck(int round, int proposer) {
  has_check_.insert(std::make_pair(round, proposer));
}

void Thunderbolt::AddIfCrossTxn(const Proposal& p) {
...
void Thunderbolt::CheckCrossRound(int round) {
...
void Thunderbolt::CheckCross(const Proposal& proposal) {
  if (proposal.header().dag_id() != dag_id_) {
    return;
  }
...
void Thunderbolt::AddBackTxn(const Transaction& tx) {
...
void Thunderbolt::SetVerifyFunc(std::function<bool(const Transaction& txn)> func) {
  verify_func_ = func;
}

void Thunderbolt::RunFutureBlock() {
  while (!IsStop()) {
    auto Check = [&]() {
...
void Thunderbolt::RunFutureCert() {
  while (!IsStop()) {
    auto Check = [&]() {
