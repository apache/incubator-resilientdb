 ...
 * under the License.
 */
#include "platform/consensus/ordering/thunderbolt/algorithm/thunderbolt.h"
...
Thunderbolt::Thunderbolt(int id, int f, int total_num, SignatureVerifier* verifier)
  ...
  send_thread_ = std::thread(&Thunderbolt::AsyncSend, this);
  commit_thread_ = std::thread(&Thunderbolt::AsyncCommit, this);
  execute_thread_ = std::thread(&Thunderbolt::AsyncExecute, this);
  cert_thread_ = std::thread(&Thunderbolt::AsyncProcessCert, this);
...
int Thunderbolt::GetLeader(int64_t r) { return r / 2 % total_num_ + 1; }

void Thunderbolt::AsyncSend() {
  ...
  while (!IsStop()) {
    ...
    queue_size_--;
    // LOG(ERROR)<<"txn before waiting time:"<<GetCurrentTime() -
    // txn->create_time()<<" queue size:"<<queue_size_;
    while (!IsStop()) {
  ...
  while (!IsStop()) {
    ...
    for (auto& it : ps) {
      for (auto& p : it.second) {
        ...
        global_stats_->AddCommitLatency(
            commit_time - p->header().create_time() - waiting_time);
...
void Thunderbolt::CommitProposal(int round, int proposer) {
...
void Thunderbolt::CommitRound(int round) {
  // LOG(ERROR)<<" commit round:"<<round;
  commit_queue_.Push(std::make_unique<int>(round));
}

bool Thunderbolt::ReceiveTransaction(std::unique_ptr<Transaction> txn) {
  //  LOG(ERROR)<<"recv txn";
...
bool Thunderbolt::ReceiveBlock(std::unique_ptr<Proposal> proposal) {
  ...
  std::unique_lock<std::mutex> lk(check_block_mutex_);
  {
    ...
    if (!CheckBlock(*proposal)) {
      ...
      // LOG(ERROR)<<"add future block:"<<proposal->header().round()<<"
      // sender:"<<proposal->header().proposer_id();
...
void Thunderbolt::ReceiveBlockACK(std::unique_ptr<Metadata> metadata) {
...
bool Thunderbolt::VerifyCert(const Certificate& cert) {
...
bool Thunderbolt::CheckBlock(const Proposal& p) {
  for (auto& link : p.header().strong_cert().cert()) {
    int link_round = link.round();
    int link_proposer = link.proposer();
    if (!proposal_manager_->CheckCert(link_round, link_proposer)) {
      // LOG(ERROR)<<" check block round:"<<p.header().round()<<"
      // proposer:"<<p.header().proposer_id()<<" strong link:"<<link_proposer<<"
      // link round:"<<link_round<<" not exist";
      return false;
    }
  }
  ...
  for (auto& link : p.header().weak_cert().cert()) {
    ...
    if (!proposal_manager_->CheckCert(link_round, link_proposer)) {
      // LOG(ERROR)<<" check block round:"<<p.header().round()<<"
      // proposer:"<<p.header().proposer_id()<<" weak link:"<<link_proposer<<"
      // link round:"<<link_round<<" not exist";
      return false;
    }
  }
  // LOG(ERROR)<<" check cert round:"<<p.header().round()<<"
  // proposer:"<<p.header().proposer_id()<<" strong link
  // size:"<<p.header().strong_cert().cert_size()<<" weak link
  // size:"<<p.header().weak_cert().cert_size();
  return true;
}

bool Thunderbolt::CheckCert(const Certificate& cert) {
...
bool Thunderbolt::SendBlockAck(std::unique_ptr<Proposal> proposal) {
  Metadata metadata;
...
void Thunderbolt::CheckFutureBlock(int round) {
  ...
  for (auto& it : hashs) {
    // LOG(ERROR)<<"send future block round:"<<it.second->header().round()<<"
    // proposer:"<<it.second->header().proposer_id();
    ...
    // std::unique_lock<std::mutex> lk(check_block_mutex_);
    {
      ...
      if (future_cert_[block_round].find(it.first) !=
        ...
        // LOG(ERROR)<<" add new cert:"<<block_round<<"
        // from:"<<cert->proposer();
...
void Thunderbolt::CheckFutureCert(int round, const std::string& hash, int proposer) {
  std::unique_lock<std::mutex> lk(future_cert_mutex_);
  auto it = future_cert_[round].find(hash);
  if (it != future_cert_[round].end()) {
    // LOG(ERROR)<<" add back future cert, round:"<<round<<"
    // proposer:"<<proposer;
...
void Thunderbolt::AsyncProcessCert() {
  while (!IsStop()) {
...
void Thunderbolt::ReceiveBlockCert(std::unique_ptr<Certificate> cert) {
  int64_t start_time = GetCurrentTime();
  if (!VerifyCert(*cert)) {
    assert(1 == 0);
    return;
...

  {
    std::unique_lock<std::mutex> lk(check_block_mutex_);
    if (!CheckCert(*cert)) {
      // LOG(ERROR)<<" add future cert round:"<<cert->round()<<"
