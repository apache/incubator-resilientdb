 ...
 * under the License.
 */
#include "platform/consensus/ordering/thunderbolt/executor/manager/sequential_concurrency_committer.h"
...
#include "platform/consensus/ordering/thunderbolt/executor/manager/mock_data_storage.h"
...
class SequentialConcurrencyCommitterTest : public Test {
 public:
  ...
  void Init() {
    ...
    committer_ = std::make_unique<SequentialConcurrencyCommitter>(
        storage_.get(), gs_.get());
...
TEST_F(SequentialConcurrencyCommitterTest, TwoTxnNoConflict) {
  ...
  // LOG(ERROR)<<"commit time:"<<committer_->GetExecutionState()->commit_time<<"
  // redo time:"<<committer_->GetExecutionState()->redo_time;
