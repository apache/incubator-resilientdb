...
bool CheckFirstCommit(const uint256_t& address, int64_t commit_id,
  ...
  if (commit_set.empty()) {
    LOG(ERROR) << "commit set is empty:" << commit_id << " is pre:" << is_pre
               << " addr:" << address;
  }
  ...
  if (*commit_set.begin() > commit_id) {
    LOG(ERROR) << "set header:" << *commit_set.begin()
               << " current:" << commit_id << " is pre:" << is_pre;
    if (is_pre == 0) {
      return false;
    }
  }
...
void AppendRecord(const uint256_t& address, int64_t commit_id,
                  XController::CommitList* commit_list) {
...
void AppendHeadRecord(const uint256_t& address, int hash_idx,
  ...
  for (auto id : commit_id) {
    // LOG(ERROR)<<"push front id:"<<id<<" idx:"<<hash_idx<<"
    // address:"<<address;
...
int64_t RemoveFromRecord(const uint256_t& address, int64_t commit_id,
                         XController::CommitList* commit_list) {
  int idx = GetHashKey(address);
  auto it = commit_list[idx].find(address);
  if (it == commit_list[idx].end()) {
...
std::vector<int64_t>& XController::GetRedo() { return redo_; }

std::vector<int64_t>& XController::GetDone() { return done_; }
...
void XController::PushCommit(int64_t commit_id,
                             const ModifyMap& local_changes) {
  int idx = commit_id & window_size_;

  // LOG(ERROR)<<"push commit:"<<commit_id<<" idx:"<<idx<<"
  // changes_list_:"<<changes_list_[idx].empty()<<" local
  // change:"<<local_changes.empty();
  ...
  for (const auto& it : change_set) {
    for (auto& op : it.second) {
      if (op.state == LOAD) {
        ...
        if (op.version != v) {
          // LOG(ERROR)<<"op log:"<<op.version<<" db ver:"<<v<<" is
          // pre:"<<is_pre<<" address:"<<it.first<<" check fail";
...
void XController::MergeChangeList(int64_t commit_id) {
  ...
  for (const auto& old_addr : changes_list_[idx]) {
    if (new_list.find(old_addr.first) == new_list.end()) {
      // int64_t  next_commit_id = RemoveRecord(old_addr.first, commit_id,
      // pre_commit_list_);
      int64_t next_commit_id =
          RemoveFromRecord(old_addr.first, commit_id, pre_commit_list_);

      // LOG(ERROR)<<" old addr:"<<old_addr.first<<" removed"<<"
      // next:"<<next_commit_id<<" curr id:"<<commit_id;
      assert(next_commit_id >= -1);
      if (next_commit_id > 0 && next_commit_id <= last_pre_commit_id_ &&
          next_commit_id > commit_id) {
        // LOG(ERROR)<<"commit id:"<<commit_id<<" remove
        // trigger:"<<next_commit_id;
...
bool XController::PreCommit(int64_t commit_id) {
  // LOG(ERROR)<<"pre commit:"<<commit_id<<" last pre
  // commit_id:"<<last_pre_commit_id_;
  if (commit_id > last_pre_commit_id_) {
    ...
    // LOG(ERROR)<<"commit :"<<commit_id<<" last :"<<last_commit_id_<<" list
    // size:"<<local_changes.size();
    for (const auto& it : local_changes) {
  ...
  // LOG(ERROR)<<"pre commit done:"<<ret<<" next:"<<last_pre_commit_id_<<"
  // ret:"<<ret;
  return ret;
}
