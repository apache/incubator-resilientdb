...
// ========================================================
std::unique_ptr<ConcurrencyController::ModifyMap> XController::GetChangeList(
    int64_t commit_id) {
  // read lock
...
void XController::StoreInternal(const int64_t commit_id,
...
uint256_t XController::LoadInternal(const int64_t commit_id,
...
void XController::Store(const int64_t commit_id, const uint256_t& address,
                        const uint256_t& value, int version) {
  StoreInternal(commit_id, address, value, version);
}

uint256_t XController::Load(const int64_t commit_id, const uint256_t& address,
                            int version) {
  return LoadInternal(commit_id, address, version);
}

bool XController::Remove(const int64_t commit_id, const uint256_t& key,
                         int version) {
...
void XController::AddDataToChangeList(std::vector<Data>& change_set,
                                      const Data& data) {
  if (change_set.empty()) {
...
void XController::ConnectSelf(int idx, int last_idx, const uint256_t& address,
                              int addr_idx) {
...
bool XController::Connect(int idx, int last_idx, const uint256_t& address,
...
int XController::FindW(int64_t commit_id, const uint256_t& address,
                       int addr_idx) {
  std::queue<int> q;
  q.push(commit_id);
...
int XController::FindR(int64_t commit_id, const uint256_t& address,
                       int addr_idx) {
  std::queue<int> q;
  q.push(commit_id);
...
void XController::AppendPreRecord(const uint256_t& address, int64_t commit_id,
                                  Data& data) {
...
const std::vector<int64_t>& XController::GetRedo() { return redo_; }
const std::vector<int64_t>& XController::GetDone() { return done_; }
