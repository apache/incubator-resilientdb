...
#include "platform/consensus/ordering/thunderbolt/executor/x_manager/dx_controller.h"
...
class DXCommitter : public ContractCommitter {
 public:
  DXCommitter(DataStorage* storage, GlobalState* global_state, int window_size,
              int worker_num = 2);
  ...
  void SetController(std::unique_ptr<DXController> controller);
 ...
 private:
  std::unique_ptr<DXController> controller_;
  // std::unique_ptr<StreamingXController> controller_;
