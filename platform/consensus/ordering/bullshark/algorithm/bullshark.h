...
class BullShark : public common::ProtocolBase {
 public:
  BullShark(int id, int f, int total_num, SignatureVerifier* verifier, const ResDBConfig& config);
  ~BullShark();
 ...
 private:
  ...
  int GetFirstLeader(int wave);
  int GetSecondLeader(int wave);
  int GetLeaderForRound(int round);
 ...
 private:
  ...
  int limit_count_;
  int commit_threshold_;  // f+1 for BullShark
