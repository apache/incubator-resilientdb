...
// Shoal++ pipelined anchors: every round has a leader (deterministic round-robin)
...
std::unique_ptr<Proposal> ProposalManager::FetchRequest(int round,
                                                         int sender) {
  std::unique_lock<std::mutex> lk(txn_mutex_);
...
void ProposalManager::GetMetaData(Proposal* proposal) {
  ...
  assert(cert_list_[round_ - 1].size() >= static_cast<size_t>(limit_count_));
...
bool ProposalManager::Ready() {
  ...
  if (static_cast<int>(cert_list_[round_ - 1].size()) < limit_count_) {
    return false;
  }
