...
class Themis : public common::ProtocolBase {
 public:
  ...
  bool ReceiveLocalOrdering(std::unique_ptr<Proposal> local_ordering);
  void ReceiveProposal(std::unique_ptr<Proposal> proposal);
  void ReceivePrepare(std::unique_ptr<Proposal> prepare);
  void ReceiveCommit(std::unique_ptr<Proposal> commit);
 ...
 private:
  ...
  std::mutex mutex_, lo_mutex_;
  std::unique_ptr<PBFT> pbft_;
