#include "platform/consensus/ordering/pbft_rl/algorithm/themis.h"
...
Themis::Themis(
  ...
  pbft_ = std::make_unique<PBFT>(id, f, total_num, verifier, 
  ...
  process_comitted_lo_thread_ = std::thread(&Themis::AsyncProcessCommittedLO, this);
}
...
bool Themis::ReceiveTransaction(std::unique_ptr<Transaction> txn) {
  ...
  return pbft_->ReceiveTransaction(std::move(txn));
}

bool Themis::ReceiveLocalOrdering(std::unique_ptr<Proposal> local_ordering) {
  return pbft_->ReceiveLocalOrdering(std::move(local_ordering));
}

void Themis::ReceiveProposal(std::unique_ptr<Proposal> proposal) {
  pbft_->ReceiveProposal(std::move(proposal));
}

void Themis::ReceivePrepare(std::unique_ptr<Proposal> prepare) {
  pbft_->ReceivePrepare(std::move(prepare));
}

void Themis::ReceiveCommit(std::unique_ptr<Proposal> commit) {
  pbft_->ReceiveCommit(std::move(commit));
}
...
bool Themis::IsCommitted(int proxy_id, uint64_t user_seq){
...
void Themis::SetCommitted(int proxy_id, uint64_t user_seq){
  committed_.insert(std::make_pair(proxy_id, user_seq));
}

bool Themis::IsSolid(const uint64_t id){
  return solid_.find(id) != solid_.end();
}

void Themis::SetSolid(uint64_t round, uint64_t id){
  if (solid_.find(id) == solid_.end()){
    solid_[id] = round;
  }
}

void Themis::SetRoundSolid(uint64_t round, uint64_t id){
...
std::vector<std::pair<uint64_t, uint64_t>> Themis::FinalOrder(Graph* g){
  //LOG(ERROR)<<" commit:"<<g;
  ...
  for(int i = 0; i <= last; ++i){
    ...
    std::unique_ptr<Transaction> data = pbft_->FetchTxn(hash);
    if(data == nullptr){
      LOG(ERROR)<<"no txn"; 
    }
...
std::set<uint64_t> Themis::set_difference(const std::set<uint64_t>& A, const std::set<uint64_t>& B) {
...
void Themis::AddImplictWeight(Graph * g_ptr, std::set<std::pair<uint64_t,uint64_t>> &possible_edges) {
...
void Themis::AddLocalOrdering(Graph * g_ptr, const std::vector<Transaction*>& txns, std::set<std::pair<uint64_t,uint64_t>> &possible_edges) {
  std::unique_lock<std::mutex> lk(lo_mutex_);
  ...
  for(uint64_t i = 0; i < non_blank_id.size(); ++i){
    ...
    if(id_2_first_g_.find(id) == id_2_first_g_.end()){
      // LOG(ERROR)<<"[F1] add new node: "<<id <<" g round: "<< round << " with: " << txn_id_proposers_[id][round].size();
  ...
  for(auto& id2: txn_ids){
    for(auto& id1: pending_txn_ids_[proposer]){
        ...
        compared_txn_ids_[id1].insert(id2);
        if (weight_[e].size() >= shaded_threshold_) {
          // LOG(ERROR) << "e with id id1: " << id1 << " id2: " << id2 << " weight: " << weight_[e];
          possible_edges.insert(e);
        }
    }
  }
...
void Themis::AddEdges(std::set<std::pair<uint64_t,uint64_t>>& possible_edges) {
...
void Themis::Push_Committed_LO (std::unique_ptr<std::vector<std::unique_ptr<Transaction>>> local_orderings) {
  committed_lo_queue_.Push(std::move(local_orderings));
}

void Themis::AsyncProcessCommittedLO () {
  while(!IsStop()){
...
void Themis::ProcessCommitedLO(const std::vector<std::unique_ptr<Transaction>> & local_orderings) {
#ifndef IMPL
  int graph_round = 0;
...
void Themis::ConstructDependencyGraph(Graph * g, const std::vector<Transaction*>& lo_txn_list) {
  assert(g!=nullptr);
  int proposal_id = -1;
...
void Themis::FindPossibleEdges(Graph* g, uint64_t id1, std::set<std::pair<uint64_t,uint64_t>>& possible_edges, std::vector<uint64_t>& ids){
  if(ids.size() == 0) {
  ...
  while(!g_queue_.empty()){
    ...
    if (txn_to_add_ids_.size() > 0) {
      ...
      for (auto &id: txn_to_add_ids_) {
        // LOG(ERROR) << "ReAdd " << id << " to round " << g->Round();
        g->AddNode(id);
        id_2_first_g_[id] = g;
        std::set<std::pair<uint64_t,uint64_t>> possible_edges;
