#include "platform/consensus/ordering/sync_fides/algorithm/sync_fides.h"
...
SyncFides::SyncFides(int id, int f, int total_num, SignatureVerifier* verifier, const ResDBConfig& config, oe_enclave_t* enclave)
  ...
  send_thread_ = std::thread(&SyncFides::AsyncSend, this);
  commit_thread_ = std::thread(&SyncFides::AsyncCommit, this);
  execute_thread_ = std::thread(&SyncFides::AsyncExecute, this);
  cert_thread_ = std::thread(&SyncFides::AsyncProcessCert, this);
...
// Sync Fides: deterministic round-robin leader.
// Synchrony assumption defeats the message-suppression attack that
// motivated random leader selection in async Fides, so we can use
// round-robin without losing safety. See design spec §7.1.
// Replicas are 1-indexed in this codebase.
// TODO(sync_fides v2): also expose a SkippedLeaders set in proposal_manager
// for explicit telemetry; v1 infers skips implicitly via previous_round.
int SyncFides::GetLeader(int r) {
  return (r % total_num_) + 1;
}

uint32_t SyncFides::TEECommonCoin(int r) {
  // Sync Fides: T-Coin removed. This stub exists only so any callers
  // that survived the fork still link. The deterministic leader in
  // GetLeader() makes the random oracle unnecessary under synchrony.
  return 0;
}

void SyncFides::AsyncSend() {
  ...
  while (!IsStop()) {
    ...
    while (!IsStop()) {
      ...
      if (!txn) {
        txn = start_ ? txns_.Pop(1) : txns_.Pop();  // 1ms timeout after startup
        if (txn) queue_size_--;
      }
      ...
      if (proposal_manager_->Ready()) {
        ...
        vote_cv_.wait_for(lk, std::chrono::microseconds(500),
    ...
    int prop_round = proposal->header().round();
    {
      ...
      *self_cert->mutable_weak_cert() = proposal->header().weak_cert();
    ...
    // Sync Fides k=1: trigger commit-check 2 rounds back so the
    // 2-hop GetReferenceNum has both round (r+1) and (r+2) populated.
    if (round > 2) {
      CommitRound(round - 2);
    }
  }
...
// Sync Fides k=1 direct commit rule (design spec §7.2 & §8.1):
//   For each round r, the deterministic leader L(r) = (r % n) + 1.
//   The leader of round r is directly committed when at least f+1
//   round-(r+1) vertices reference v_L via their strong_cert (1-hop).
//   This is the theoretical 2Δ commit latency the spec promises.
//
//   Commit latency: 2 rounds (leader at r → wave at r+1 → commit).
//   Worst case under f Byzantine leaders: (f+1)·Δ + Δ rounds.
//
//   Skipped leaders are pulled in via recursive commit when a future
//   leader is committed and its causal history walks back.
//
// We use 1-hop instead of the 2-hop GetReferenceNum that async Fides
// uses. 1-hop is safe under sync + MC because (a) MC prevents
// equivocation so each replica's round-r vertex is unique, and (b)
// f+1 honest references guarantees agreement.
void SyncFides::AsyncCommit() {
  ...
  while (!IsStop()) {
    ...
    // The popped value is the leader-round upper bound to check,
    // chosen by AsyncSend so that round (r+1) already exists when
    // we evaluate leader at round r.
    int target_leader_round = *round_or;

    for (int r = previous_round + 1; r <= target_leader_round; ++r) {
      int leader = GetLeader(r);

      // Brief wait for the leader vertex if it hasn't arrived yet
      // (out-of-order delivery races). Under synchrony, the leader
      // vertex is almost always present 1 round later.
      const Proposal* req = proposal_manager_->GetRequest(r, leader);
      for (int i = 0; req == nullptr && i < 5 && !IsStop(); ++i) {
        std::this_thread::sleep_for(std::chrono::microseconds(100));
        req = proposal_manager_->GetRequest(r, leader);
      }

      if (req == nullptr) {
        // Leader vertex missing — skipped. Recursive commit will
        // pull it in later via the causal history of a future
        // committed leader.
        continue;
      }

      // 1-hop direct reference count (counts both strong_cert and
      // weak_cert refs from any future round, populated in
      // ProposalManager::AddCert). This is the design spec §7.2 rule
      // and gives the theoretical 2Δ commit latency.
      int reference_num =
          proposal_manager_->GetDirectReferenceNum(r, leader);
      // Byzantine-leader fault injection (mirror of Fides's knob).
      if (leader <= static_cast<int>(config_.GetFailureNum())) {
        reference_num = 0;
      }

      if (reference_num < limit_count_) {
        // Not enough direct references — skip this leader.
        continue;
      }

      CommitProposal(r, leader);
    }

    previous_round = target_leader_round;
  }
}

// TODO: 3.Decrypt the request before executing.
void SyncFides::AsyncExecute() {
...
void SyncFides::CommitProposal(int round, int proposer) {
...
void SyncFides::CommitRound(int round) {
  //LOG(ERROR)<<" commit round:"<<round;
  commit_queue_.Push(std::make_unique<int>(round));
}

bool SyncFides::ReceiveTransaction(std::unique_ptr<Transaction> txn) {
...
bool SyncFides::ReceiveBlock(std::unique_ptr<Proposal> proposal) {
  ...
  *cert->mutable_weak_cert() = proposal->header().weak_cert();
...
void SyncFides::ReceiveBlockACK(std::unique_ptr<Metadata> metadata) {
  ...
  if (received_num_[hash].size() == limit_count_) {
    ...
    *cert.mutable_weak_cert() = p->header().weak_cert();
...
bool SyncFides::VerifyCert(const Certificate & cert){
  if (config_.GetMCEnabled())
    return true;
  if (cert.round() == 0){
    return true;
...
bool SyncFides::VerifyCertSimulation(const Certificate & cert){
  if (config_.GetMCEnabled())
    return true;
  if (cert.round() == 0){
    return true;
...
bool SyncFides::VerifyCounter(int round, const CounterInfo& counter) {
...
bool SyncFides::CheckBlock(const Proposal& p){
...
bool SyncFides::CheckCert(const Certificate& cert){
...
bool SyncFides::SendBlockAck(std::unique_ptr<Proposal> proposal) {
...
void SyncFides::CheckFutureBlock(int round){
...
void SyncFides::CheckFutureCert(int round, const std::string& hash, int proposer) {
  std::unique_lock<std::mutex> lk(future_cert_mutex_);
...
void SyncFides::AsyncProcessCert(){
  while(!IsStop()){
...
void SyncFides::ReceiveBlockCert(std::unique_ptr<Certificate> cert) {
  int64_t start_time = GetCurrentTime();
  if(!VerifyCert(*cert)){
...
bool SyncFides::GetEnclaveStatOnce(uint64_t* current, uint64_t* peak, uint64_t* max)
{
...
void SyncFides::StartEnclaveStatMonitor(const std::string& log_path)
{
...
void SyncFides::StopEnclaveStatMonitor()
{
    std::lock_guard<std::mutex> lock(sampler_mutex_);
...
bool SyncFides::IsByzantine() const {
  return config_.GetByzantineMode() > 0;
}

std::vector<int> SyncFides::GetByzantineTargetNodes() const {
...
void SyncFides::ByzantineBroadcast(int msg_type, const google::protobuf::Message& msg) {
