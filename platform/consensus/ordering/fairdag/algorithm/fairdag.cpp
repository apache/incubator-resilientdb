...
void FairDAG::SortTxn(std::vector<std::unique_ptr<Transaction>>& txns,
  ...
  auto cmp = [&](std::unique_ptr<Transaction>& txn1,
...
void FairDAG::CommitTxns(std::vector<std::unique_ptr<Transaction>>& txns) {
  ...
  for (auto& txn : txns) {
    ...
    committed_txns_[key].insert(
        std::make_pair(txn->proposer(), txn->timestamp()));
    if (committed_time_.find(key) == committed_time_.end()) {
      committed_time_[key] = commit_time;
    }
    min_timestamp_[txn->proposer()] =
        std::max(min_timestamp_[txn->proposer()], txn->timestamp() + 1);
    // LOG(ERROR)<<"proposer:"<<txn->proposer()<<" next commit
    // time:"<<min_timestamp_[txn->proposer()]<<" timestamp:"<<txn->timestamp();
  }
  ...
  for (auto& txn : txns) {
    ...
    // LOG(ERROR)<<"commit from:"<<txn->proxy_id()<<" user
    // seq:"<<txn->user_seq()<<" time num:"<<committed_txns_[key].size()<<"
    // proposer:"<<txn->proposer()<<" timestamp:"<<txn->timestamp()<<"
    // lowerbound:"<<lower_bound_uncommtted_time<<" commit time:"<<commit_time<<"
    // first time:"<<committed_time_[key];

    if (static_cast<int>(committed_txns_[key].size()) < 2 * f_ + 1) {
      uint64_t lower_bound_time = GetLowerboundTimestamp(key);
      lower_bound_uncommtted_time =
          std::min(lower_bound_uncommtted_time, lower_bound_time);
    } else {
  ...
  for (auto it = not_ready_.begin(); it != not_ready_.end();) {
    if (it->first < lower_bound_uncommtted_time) {
      // std::pair<int,int64_t> key = std::make_pair(it->second->proxy_id(),
      // it->second->user_seq()); LOG(ERROR)<<" get from pending:
      // proposer:"<<it->second->proposer()<<" assign time:"<<it->first<<" user
      // seq:"<<it->second->user_seq();
  ...
  for (auto& txn : execute_txns) {
    std::pair<int, int64_t> key =
        std::make_pair(txn->proxy_id(), txn->user_seq());
    // LOG(ERROR)<<"commit from proposer:"<<txn->proxy_id()<<"
    // time:"<<txn->timestamp()<<" seq:"<<txn->user_seq()<<"
    // proposer:"<<txn->proposer()<<" commit time:"<<commit_time<<" first
    // time:"<<committed_time_[key]<<" delay:"<<(GetCurrentTime() -
    // committed_time_[key]);
    ...
    while (true) {
      ...
      if (data == nullptr) {
        LOG(ERROR) << "no txn from:" << txn->proxy_id()
                   << " user seq:" << txn->user_seq();
      }
      assert(data != nullptr);
      data->set_id(execute_id_++);
      // LOG(ERROR)<<" data queuing time:"<<data->queuing_time()<<" txn queuing
      // time:"<<txn->queuing_time();

      // global_stats_->AddCommitDelay(commit_time- data->create_time() -
      // data->queuing_time());
  ...
  // LOG(ERROR)<<"exe time:"<<execute_txns.size()<<" total:"<<txns.size()<<"
  // ratio:"<<ratio;
