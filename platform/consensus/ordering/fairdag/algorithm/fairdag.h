...
private:
  void OrderInCausalHistory(std::vector<std::unique_ptr<Transaction> >& txns);
  ...
  void CollectTimestamp(const Proposal *p);

 private:
  std::mutex mutex_, ts_mutex_;
  std::unique_ptr<Tusk> tusk_;
  int64_t local_time_ = 0;
  std::map<std::pair<int,int64_t>, std::map<int, uint64_t> > committed_txns_;
  std::map<std::pair<int,int64_t>, std::map<int, uint64_t> > uncommitted_txns_;
  ...
  Stats* global_stats_;
  std::map<std::pair<int,int64_t>, int64_t> assigned_ts_;
  std::set<std::pair<int,int64_t> > committed_;
};
