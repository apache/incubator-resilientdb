...
void ProposalManager::AddTxn(const std::string& hash,
                             std::unique_ptr<Transaction> txn) {
  std::unique_lock<std::mutex> lk(data_mutex_);
  data_[hash] = std::move(txn);
}

std::unique_ptr<Transaction> ProposalManager::FetchTxn(
  ...
  // LOG(ERROR)<<" fetch txn from:"<<it->second->proxy_id()<<"
...
std::unique_ptr<Transaction> ProposalManager::FetchTxnCopy(
    const std::string& hash) {
  std::unique_lock<std::mutex> lk(data_mutex_);
  auto it = data_.find(hash);
  if (it == data_.end()) {
...
std::unique_ptr<Proposal> ProposalManager::GenerateProposal(
  ...
  std::string data;
  {
    for (auto& hash : txns) {
      ...
      data += *hash;
      // LOG(ERROR)<<"add data from :"<<txn->proxy_id()<<"
      // seq:"<<txn->user_seq()<<" ts:"<<digest->ts()<<"
      // queuing_time:"<<txn->queuing_time()<<" txn create
      // time:"<<(txn->create_time())<<" queueing time:"<<txn->queuing_time()<<"
      // proposal create time:"<<proposal->header().create_time();
    }
