#include "platform/consensus/ordering/hs1/algorithm/hs1.h"
...
HotStuff1::HotStuff1(int id, int f, int total_num, SignatureVerifier * verifier, int non_responsive_num, int fork_tail_num, int rollback_num, uint64_t timer_length)
  : ProtocolBase(id, f, total_num), verifier_(verifier), non_responsive_num_(non_responsive_num), fork_tail_num_(fork_tail_num), rollback_num_(rollback_num){

    LOG(ERROR)<<"id:"<<id<<" f:"<<f<<" total:"<<total_num_;

  global_stats_ = Stats::GetGlobalStats();
  proposal_manager_ = std::make_unique<ProposalManager>(id, 2*f_+1, verifier, total_num, non_responsive_num, fork_tail_num, rollback_num, timer_length);
  has_sent_ = false;
    send_thread_ = std::thread(&HotStuff1::AsyncSend, this);
    commit_thread_ = std::thread(&HotStuff1::AsyncCommit, this);
    batch_size_ = 1;
...
void HotStuff1::AsyncSend() {
  while (!IsStop()) {
    ...
    std::unique_ptr<Proposal> proposal2 = nullptr;
    {
      ...
      if(id_ < 3 * rollback_num_ && id_ % 3 ==1) {
        proposal2 = proposal_manager_ -> GenerateFakeProposal(txns);
      } 
...
bool HotStuff1::ReceiveTransaction(std::unique_ptr<Transaction> txn) {
...
bool HotStuff1::ReceiveProposal(std::unique_ptr<Proposal> proposal) {
  ...
  {
    // LOG(ERROR)<<"[X]process proposer view:"<<view<< " at " << (GetCurrentTime() - last_time_) << " " << GetCurrentTime();
...
bool HotStuff1::ReceiveCertificate(std::unique_ptr<Certificate> cert) {
...
void HotStuff1::CommitProposal(std::unique_ptr<Proposal> p){
  commit_q_.Push(std::move(p));
}

std::unique_ptr<Certificate> HotStuff1::GenerateCertificate(const Proposal& proposal) {
