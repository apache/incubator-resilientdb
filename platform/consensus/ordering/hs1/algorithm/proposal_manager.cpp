...
ProposalManager::ProposalManager(int32_t id, int limit_count, SignatureVerifier* verifier, int total_num, int non_responsive_num, int fork_tail_num, int rollback_num, uint64_t timer_length)
    : id_(id), limit_count_(limit_count), verifier_(verifier), total_num_(total_num), non_responsive_num_(non_responsive_num), fork_tail_num_(fork_tail_num), rollback_num_(rollback_num) {
    round_ = 1;
...
std::vector<std::unique_ptr<Proposal>> ProposalManager::AddProposal(std::unique_ptr<Proposal> proposal){
  ...
  const Proposal * fafather = nullptr;
  father = GetProposal(proposal->header().prehash());
  if(father != nullptr){ // Attempt to Speculate on father
    //LOG(ERROR)<<"get father view:"<<father->header().view();
    lock_qc_ = father->header().qc();
    fafather = GetProposal(father->header().prehash());
    if (father->header().view() == proposal->header().view() - 1) { // No-Gap Rule
      if (fafather == nullptr || fafather->header().view() == father->header().view() - 1) {  // Prefix Speculation Rule
        auto fetched_proposal = FetchProposal(proposal->header().prehash());
    ...
    } else {  // Attempt to Commit fafather
