...
OOOHotStuff::OOOHotStuff(int id, int f, int total_num,
  ...
  proposal_manager_ =
      std::make_unique<ProposalManager>(id, 2 * f_ + 1, slot_num_, verifier);
...
bool OOOHotStuff::IsLeader(int view) { return (view % total_num_) + 1 == id_; }

bool OOOHotStuff::Ready(int k) {
  int view = proposal_manager_->CurrentView(k);
  // LOG(ERROR)<<"get view:"<<view<<" from slot:"<<k<<" has
  // sent:"<<has_sent_[k];
  ...
  while (!IsStop()) {
    ...
    while (true) {
      if (pending_commit_.find(next_execute_key) != pending_commit_.end()) {
        ...
        // view:"<<exe_p->second->header().view()<<"
        // slot:"<<exe_p->second->header().slot();
...
bool OOOHotStuff::ProcessProposal(std::unique_ptr<Proposal> proposal) {
  ...
  std::unique_ptr<Certificate> cert = nullptr;
  {
    ...
    std::vector<std::unique_ptr<Proposal>> committed_list =
    ...
    // LOG(ERROR)<<"send cert view:"<<view<<" slot:"<<slot<<"
    // to:"<<NextLeader(view)<<" commit proposal size:"<<committed_list.size();
  }
...
bool OOOHotStuff::ReceiveCertificate(std::unique_ptr<Certificate> cert) {
  ...
  // LOG(ERROR)<<"RECEIVE proposer cert, view :"<<view<<" slot:"<<slot<<"
  // size:"<<receive_[key][hash].size();
...
std::unique_ptr<Certificate> OOOHotStuff::GenerateCertificate(
