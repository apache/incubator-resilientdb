...
ProposalManager::ProposalManager(int32_t id, int limit_count, int slot_num,
...
bool ProposalManager::VerifyQC(const QC& qc) {
  if (qc.signatures_size() < limit_count_) {
    LOG(ERROR) << "qc size:" << qc.signatures_size()
               << " not enought, limit:" << limit_count_;
    return false;
  }
...
bool ProposalManager::SafeNode(const Proposal& proposal) {
  ...
  // LOG(ERROR)<<"qc view:"<<proposal.header().qc().view()<<" lock
  // view:"<<lock_qc_[slot].view()<<" safe fail";
  return false;
}
...
std::unique_ptr<Proposal> ProposalManager::GenerateProposal(
  ...
  std::unique_ptr<Proposal> proposal = std::make_unique<Proposal>();
  {
    ...
    if (!generic_qc_[slot].hash().empty()) {
      // LOG(ERROR)<<" new proposal add qc:"<<generic_qc_[slot].view()<<"
      // slot:"<<generic_qc_[slot].slot()<<" get slot:"<<slot;
...
int ProposalManager::CurrentView(int slot) { return round_[slot]; }
...
void ProposalManager::UpdateGenericQC(const QC& qc) {
  ...
  for (int i = 0; i <= slot; ++i) {
    if (generic_qc_[i].view() == 0 || generic_qc_[i].view() < view) {
      ...
      // LOG(ERROR)<<"get new round:"<<round_[i]<<" slot:"<<i<<" gene
      // view:"<<generic_qc_[i].view()<<" slot:"<<generic_qc_[i].slot();
    }
  }
  ...
  /*
    for(int i = 0; i < slot_num_; ++i){
      if(generic_qc_[i].view()>0){
        LOG(ERROR)<<"slot i:"<<i<<" view:"<<generic_qc_[i].view()<<"
    slot:"<<generic_qc_[i].slot(); assert(generic_qc_[i].slot() >=i);
      }
    }
    */
}

std::vector<std::unique_ptr<Proposal>> ProposalManager::AddProposal(
  ...
  if (father != nullptr) {
    ...
    if (fafather != nullptr) {
      ...
      if (fafafather != nullptr) {
        ...
        // slot:"<<fafafather->header().slot();
        if (next_commit_[view] <= fafafather->header().slot()) {
          for (int i = next_commit_[view]; i <= fafafather->header().slot();
               ++i) {
            std::unique_ptr<Proposal> commit_ready_proposal =
                FetchProposal(fafafather->header().view(), i);
            if (commit_ready_proposal != nullptr) {
              commit_ready_proposal_list.push_back(
                  std::move(commit_ready_proposal));
            }
          }
  ...
  assert(local_block_hash_.find(std::make_pair(view, slot)) ==
         local_block_hash_.end());
