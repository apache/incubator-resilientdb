...
3. ResilientDB supports a primary-backup architecture where one replica is designated as the  **primary** (replicas with identifier **0**) and is responsible for initiating the consensus; backups aim to follow a non-malicious primary.
4. ResilientDB exposes a wide range of interfaces such as a **Key-Value** store, **Smart Contracts**, **UTXO**, and **Python SDK**. Following are some of the decentralized applications (DApp) that have been built on top ResilientDB: **[NFT Marketplace](https://nft.resilientdb.com/)** and **[Debitable](https://debitable.resilientdb.com/)**.
